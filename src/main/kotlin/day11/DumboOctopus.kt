...
class DumboOctopus(fileName: String) : Solution<List<Int>, Long>(fileName) {
    ...
    override fun List<List<Int>>.solve2(): Long {
        ...
        tailrec fun runSteps(currentStep: Long, flashed: Long, g: Grid<Int>): Long {
            return if (g.all { it.value == 0 }) currentStep else {
                val (flashStep: Long, newGrid: Grid<Int>) = runStep(g)
                runSteps(currentStep + 1, flashed + flashStep, newGrid)
            }
        }
    ...
    private tailrec fun flash(g: Grid<Int>, flashed: Set<Point>): Pair<Set<Point>, Grid<Int>> {
        val toFlash = g
            .filterValues { it > 9 }
    ...
    private fun runStep(g: Grid<Int>): Pair<Long, Grid<Int>> {
